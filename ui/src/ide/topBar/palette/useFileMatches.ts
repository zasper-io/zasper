...
import { ContentEntry, searchFiles } from '@/api';
 ...
 * without a request.
 */
export function useFileMatches(query: string): ContentEntry[] {
  const [matches, setMatches] = useState<ContentEntry[]>([]);
  const cache = useRef<Record<string, ContentEntry[]>>({});
