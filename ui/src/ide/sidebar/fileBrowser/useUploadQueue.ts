...
export interface Upload extends PendingUpload {
  state: UploadState;
...
export interface UploadQueue {
  ...
  add: (pending: PendingUpload[]) => void;
...
const queued = (pending: PendingUpload): Upload => ({
  ...pending,
  state: 'waiting',
  progress: 0,
  reason: '',
 ...
 */
export function useUploadQueue(parentDir: string, initial: PendingUpload[]): UploadQueue {
  const [uploads, setUploads] = useState<Upload[]>(() => initial.map(queued));
  ...
  useEffect(() => {
    const change = (relativePath: string, to: Partial<Upload>) =>
      setUploads((all) =>
