...
export function pendingFromFiles(files: FileList | null): PendingUpload[] {
 ...
 */
export function pendingFromDrop(transfer: DataTransfer): Promise<PendingUpload[]> {
...
async function collect(entries: FileSystemEntry[], prefix: string): Promise<PendingUpload[]> {
  const found = await Promise.all(entries.map((entry) => walk(entry, prefix)));
  return found.flat();
}

async function walk(entry: FileSystemEntry, prefix: string): Promise<PendingUpload[]> {
