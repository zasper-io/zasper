...
import ChangeList, { ChangeAction } from './ChangeList';
...
import { GitPanelStatus, useGitStatus } from './useGitStatus';
...
export default function GitPanel({ hidden, reveal }: GitPanelProps) {
  ...
  const changesHistory: GitPanelStatus['run'] = useCallback(
  ...
  const stage: ChangeAction = {
    label: 'Stage',
    icon: 'plus',
    run: (paths) => void run(() => stageFiles(paths)),
  };
  const unstage: ChangeAction = {
    label: 'Unstage',
    icon: 'minus',
    run: (paths) => void run(() => unstageFiles(paths)),
  };
  const discard: ChangeAction = {
    label: 'Discard',
    icon: 'undo-2',
