...
import { CommitAction } from './useCommitAction';
import { GitPanelStatus } from './useGitStatus';

/** What the panel lends its commands: its one way of writing, and its two ways of being looked at. */
export interface GitCommandTargets {
  busy: boolean;
  run: GitPanelStatus['run'];
  commit: CommitAction;
 ...
 */
export function useGitCommands(targets: GitCommandTargets): Command[] {
  const { busy, run, commit } = targets;

  const gitCommand = (command: Omit<Command, 'category' | 'scope' | 'isEnabled'>): Command => ({
    category: 'Git',
    scope: 'app',
