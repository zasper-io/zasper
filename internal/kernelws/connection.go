...
// How often the handshake asks the kernel again. Its overall budget is kernel.KernelStartupBudget, the same one
...
func (kwsConn *Connection) signals() (ready, iopubSeen chan struct{}) {
	kwsConn.mu.Lock()
...
func (kwsConn *Connection) iopubArrived() {
...
func (kwsConn *Connection) waitReady() bool {
...
func (kwsConn *Connection) stopPolling() {
...
func (kwsConn *Connection) pollChannel(socket zmq4.Socket, socketName string) {
	kwsConn.mu.Lock()
...
func (kwsConn *Connection) Prepare(sessionId string) {
	kwsConn.Session = kwsConn.KernelManager.Session
}

func (kwsConn *Connection) Connect() {
...
func (kwsConn *Connection) nudge() {
	...
	deadline := time.After(kernel.KernelStartupBudget)
	...
	for replied != nil || iopubSeen != nil {
		...
		case <-deadline:
			log.Warn().Msgf("kernel %s did not answer within %s; using the connection anyway",
				kwsConn.KernelId, kernel.KernelStartupBudget)
			return
		case <-kwsConn.Context.Done():
			return
		}
...
func (kwsConn *Connection) handleIncomingMessage(incomingMsg []byte) {
...
func (kwsConn *Connection) ReadMessagesFromClient(waiter *sync.WaitGroup) {
	defer func() {
...
func (kwsConn *Connection) WriteMessages(waiter *sync.WaitGroup) {
	defer func() {
		kwsConn.Conn.Close()
		kwsConn.stopPolling()
		waiter.Done()
